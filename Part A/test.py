...
def train_diffusion_model(model, dataloader, optimizer, num_epochs=10):
    ...
    for epoch in range(num_epochs):
        ...
        for i, (x, t) in enumerate(dataloader):
            ...
            target = x if model.predict_x0 else torch.randn_like(x)
...
# Example usage
def main():
    # Generate some synthetic data for the example (replace with your real dataset)
    batch_size = 16
    x_data = torch.randn(100, 1, 28, 28)  # Example image data (100 images of 28x28)
    t_data = torch.randint(0, 100, (100,))  # Time steps (t)
    ...
    model_noise = ReverseDiffusionPrediction(network=ScoreNet(marginal_prob_std=1.0), predict_x0=False)
    model_x0 = ReverseDiffusionPrediction(network=ScoreNet(marginal_prob_std=1.0), predict_x0=True)
    ...
    train_diffusion_model(model_noise, dataloader, optimizer_noise, num_epochs=5)

    print("Training model for x0 prediction...")
    train_diffusion_model(model_x0, dataloader, optimizer_x0, num_epochs=5)

    # Evaluate both models
    print("\nEvaluating model for noise prediction (epsilon)...")
    loss_noise = evaluate_diffusion_model(model_noise, dataloader)

    print("\nEvaluating model for x0 prediction...")
    loss_x0 = evaluate_diffusion_model(model_x0, dataloader)

    # Visualize comparison
    print("\nVisualizing the comparison between the two models...")
    visualize_comparison(model_noise, model_x0, dataloader)
